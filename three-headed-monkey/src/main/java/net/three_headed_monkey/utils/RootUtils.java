...
public class RootUtils {
    ...
    public static final String SETTINGS_BACKUP_FILENAME = "/system/etc/3hm-settings-backup.xml";
    public static final String SET_SETTINGS_BACKUP_FILE_PERMISSIONS_COMMAND = "chmod 0664 " + SETTINGS_BACKUP_FILENAME;
    ...
    public void backupSettingsFile(){
        ...
        if(!backup_settings_file.exists()){
            Shell.SU.run(new String[] {"touch " + SETTINGS_BACKUP_FILENAME, SET_SETTINGS_BACKUP_FILE_PERMISSIONS_COMMAND});
        }
    ...
    public boolean isSystemMountedReadOnly(){
        List<String> mount_output = Shell.SH.run(CURRENT_MOUNTS_COMMAND);
